...
const poll = {
  ...
  registerNewAnswers() {
    ...
    if (typeof question === 'number' && question >= 0 && question <= 3) {
      this.answers[question]++;
      const outputType = Number(
        prompt(`What type of output do you want?
      0: Default
      1: String`)
      );

      if (
        typeof outputType === 'number' &&
        outputType >= 0 &&
        outputType <= 1
      ) {
        if (outputType === 0) {
...
const registerNewAnswers = poll.registerNewAnswers.bind(poll);
...
const poll = {
  ...
  displayResults(type = 'array') {
    if (type === 'array') console.log(this.answers);
      ...
      let output = `Poll results are ${this.answers.join(', ')}`;
      console.log(output);
    }
  },
};
